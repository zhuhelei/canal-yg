const { createApp, computed, nextTick, onMounted, reactive, ref } = Vue
...
createApp({
  setup() {
    ...
    async function loadList() {
      ...
      if (!selectedStillExists) await openDetail(items.value[0].id)
    }
    ...
    function createSatelliteLayer() {
      ...
      const url = 'https://ditu.zjzwfw.gov.cn/services/wmts/imgmap/default/oss?service=WMTS&request=GetTile&version=1.0.0&layer=mapSatellite&style=&tilematrixset=imgmap&format=image%2Fjpeg&height=256&width=256&tilematrix={z}&tilerow={y}&tilecol={x}&token=051eb92d-6fba-426f-9de7-14e786d3fa9a'
      return new L.TileLayer.ArcGIS(url, {
        zoomOffset: 0,
        maxZoom: 21,
        minZoom: 10,
        noWrap: true,
    ...
    <main class="page shell-page">
      ...
      <section class="main-layout">
        <aside class="record-rail">
          <section class="rail-filters">
            <label>月份<input v-model="query.month" type="month" /></label>
            <label>所属区域<input v-model="query.regionName" placeholder="输入区域名称" /></label>
            <label>
              核验状态
      ...
      <div v-if="showImportDialog" class="dialog-mask">
        <form class="dialog" @submit.prevent="importZip">
          ...
          <label>月份<input v-model="importForm.month" type="month" required /></label>
          <label>ZIP 包<input type="file" accept=".zip,application/zip" required @change="pickZip" /></label>
